export const deleteAll = async (client, childDbName) => {
    ...
    if (typeof childDbName !== "undefined" && childDbName !== "") {
        ...
        await handleSetupError(client.query(Map(Paginate(Documents(Keys())), (x) => Let({
            key: Get(x),
            ref: Select(["ref"], Var("key")),
            db: Select(["database"], Var("key"), "none"),
        }, If(Equals(Var("db"), Database(childDbName)), Delete(Var("ref")), false)))), "delete keys - delete keys linked to database");
        await handleSetupError(client.query(If(Exists(Database(childDbName)), Delete(Database(childDbName)), false)), "database - delete child database");
    }
    else {
        try {
...
export const DeleteAll = async (client) => {
    return client.query(Map(Paginate(Collections()), Lambda("ref", Map(Paginate(q.Documents(Var("ref"))), Lambda("ref2", Delete(Var("ref2")))))));
};
export const DeleteAllUsers = async (client) => {
    return client.query(Map(Paginate(q.Documents(Collection('users'))), Lambda("ref2", Delete(Var("ref2")))));
};
import "dotenv/config";
import { handleSetupError } from "./errors.js";
import faunadb from "faunadb";
const q = faunadb.query;
const { Exists, If, Database, Map, Collections, Collection, Documents, Paginate, Lambda, Delete, Var, Let, Get, Select, Equals, Keys, } = faunadb.query;
