...
export const generalTestSetup = async function (dbName) {
    ...
    if (args.fullSetup) {
        ...
        const secret = await handlePromiseError(deleteAndCreateDatabase(adminClientParentDb, dbName), `Creating ${dbName} database`);
        await countdown("Countdown Post Destroy", 61).then(() => {
            console.log("FINISHED COUNTDOWN");
        });
    }
    const adminKey = await handleSetupError(adminClientParentDb.query(CreateKey({ database: q.Database(dbName), role: "admin" })), "Admin key - child db");
...
const { CreateKey } = q;
import "dotenv/config";
import { setupDatabase, deleteAndCreateDatabase, } from "./database.js";
import { handlePromiseError, handleSetupError, } from "./errors.js";
